 ...
 *
 * Rename 参数（统一 {} 语法）
 * - [format] 格式化模板，默认: {region_code} {isp_code}
 *           可用占位符:
 *             {region_code}      - 节点名检测到的区域代码 (如 HK, JP)
 ...
 *          兼容旧语法: region_code(HK,JP) ASC, tag(IPLC) DESC
 *          字段: region_code, region_name, region_flag, isp_code, isp_name, tag, index, name, latency
 *          升序/降序: ASC (默认) / DESC
...
const node_timeout = parseFloat(args.timeout ?? 1000); // 单节点超时 ms
...
// Rename 配置
// ============================================================
const format = args.format || '{region_code} {isp_code}';
const connector = args.Connector || args.connector || ' ';
const sort = args.Sort || args.sort || null;
const remove_failed = args.remove_failed !== false; // 默认 true
...
async function operator(proxies = []) {
    ...
    const { ports, pid, meta_ports } = await probeAll(internalProxies, proxies, (ip, result) => {
        if (result) {
            probeSuccess++;
        } else {
            probeFail++;
    ...
    async function probeAll(internalProxies, proxies, onResult) {
        ...
        if (needsMeta.length === 0) {
            return { ports: null, pid: null, meta_ports: {} };
        }
...
function applyFormat(proxy, formatStr, connectorStr) {
    ...
    for (const part of parts) {
        if (part.type === 'text') {
            ...
            const v = resolvePlaceholder(proxy, part.value, conn);
            resultParts.push(v);
        }
    }
...
function resolvePlaceholder(proxy, placeholder, conn) {
    ...
    if (placeholder === 'latency') {
        return String(geo.latency !== undefined && geo.latency !== null ? geo.latency : 0);
    }

    // {index:2d} / {index:3d} 格式填充
    if (placeholder.startsWith('index')) {
        if (placeholder.includes(':')) {
            const width = parseInt(placeholder.split(':')[1].replace('d', ''));
            return String(proxy.index || 0).padStart(width, '0');
        }
        return String(proxy.index || 0);
    }

    // {tag:XXX} 动态检测：有 XXX 关键词则输出 XXX
    ...
    // {iplc} 有 IPLC/专线 关键词输出 IPLC
    if (placeholder === 'iplc') {
        return /iplc|专线/i.test(lowerName) ? 'IPLC' : '';
    }
    // {udpn}
    if (placeholder === 'udpn') {
        return /udpn/i.test(lowerName) ? 'UDPN' : '';
    }
    // {home}
    if (placeholder === 'home') {
        return /家宽|home/i.test(lowerName) ? 'Home' : '';
    }
...
function parseSortRules(sortString) {
    ...
    for (const part of parts) {
        ...
        // 解析 field + optional(values) + optional(ASC/DESC)
        // 新语法: {region_code(HK,JP)} ASC  /  {region_code} DESC  /  {tag(IPLC)} DESC
        // 旧语法: region_code(HK,JP) ASC   /  region_code ASC
        ...
        if (newMatch) {
            // 新语法 {field} 或 {field(values)}  + 可选 ASC/DESC
            const inside = newMatch[1]; // "region_code(HK,JP)" 或 "tag" 或 "tag(IPLC)"
