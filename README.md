...
│   └── scripts/        # 节点处理脚本
│       ├── region-name-match.js    # 地区名称匹配（推荐）
│       └── region-english.js       # IP 地理位置查询
...
2. **格式化已有 region 的 countryCode**：统一不规范的 region 值为英文标准格式

使用方法：
1. 在 Sub Store 订阅中添加"操作器"
2. 选择"脚本操作器"
3. 上传 `sub-store/scripts/region-name-match.js`
4. 配置参数：
   - `removeEmoji`: true（移除 emoji）
   - `useFullName`: true（使用完整国家名）
